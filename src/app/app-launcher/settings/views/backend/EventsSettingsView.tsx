import React, { useState, useEffect } from 'react';
import SettingsSection from '../../../../../components/app-launcher/settings/SettingsSection';
import Toggle from '../../../../../components/Toggle';
import { CalendarIcon, ChartBarIcon } from '../../../../../icons';
import { getEventsSettings, saveEventsSettings, EventsSettings } from '../../../../api/settings/eventsApi';
import AppContentHeader from '../../../../../components/app-launcher/shared/AppContentHeader';

const EventsSettingsView: React.FC = () => {
  const [settings, setSettings] = useState<EventsSettings | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState('management');

  useEffect(() => {
    const fetchSettings = async () => {
      try {
        setLoading(true);
        const data = await getEventsSettings();
        setSettings(data);
      } catch (err) {
        setError("Failed to load settings.");
      } finally {
        setLoading(false);
      }
    };
    fetchSettings();
  }, []);

  const handleSave = async () => {
    if (settings) {
      await saveEventsSettings(settings);
      alert("Settings saved!");
    }
  };

  const handleChange = (key: keyof EventsSettings, value: string | boolean | number) => {
    setSettings(prev => prev ? { ...prev, [key]: value } : null);
  };
  
  const tabs = [
      { id: 'management', label: 'Management' },
      { id: 'integrations', label: 'Integrations' },
  ];

  if (loading) {
    return <div className="flex items-center justify-center h-full"><ChartBarIcon className="w-12 h-12 text-gyn-text-primary-light dark:text-gyn-text-primary-dark animate-pulse" /></div>;
  }
  
  if (error || !settings) {
    return <div className="p-8 text-center text-gyn-text-error-light dark:text-gyn-text-error-dark">{error || 'Settings could not be loaded.'}</div>;
  }

  return (
    <div className="p-4 md:p-8 text-gyn-text-primary-light dark:text-gyn-text-primary-dark">
      <div className="flex items-center gap-3 mb-6">
        <CalendarIcon className="w-8 h-8 text-gyn-text-secondary-light dark:text-gyn-text-secondary-dark" />
        <h2 className="text-2xl font-bold text-gyn-text-secondary-light dark:text-gyn-text-secondary-dark">Events Backend Settings</h2>
      </div>

      <AppContentHeader tabs={tabs} activeTab={activeTab} setActiveTab={setActiveTab} />
      
      <div className="mt-6">
        {activeTab === 'management' && (
          <SettingsSection
            title="Event Management"
            description="Control event submissions and financial settings."
          >
            <div className="flex justify-between items-center py-2 border-b border-gyn-border-primary-light dark:border-gyn-border-primary-dark">
              <div>
                <p className="font-semibold text-gyn-text-secondary-light dark:text-gyn-text-secondary-dark">Allow Public Event Submissions</p>
                <p className="text-sm">Permit users to submit their own events for listing on the platform.</p>
              </div>
              <Toggle checked={settings.allowSubmissions} onChange={(val) => handleChange('allowSubmissions', val)} />
            </div>
            <div className="py-2 mt-4">
              <label className="font-semibold text-gyn-text-secondary-light dark:text-gyn-text-secondary-dark" htmlFor="service-fee">Ticketing Service Fee (%)</label>
              <p className="text-sm mb-2">A percentage fee added to all ticket sales processed through the platform.</p>
              <input
                id="service-fee"
                type="number"
                value={settings.serviceFee}
                onChange={(e) => handleChange('serviceFee', Number(e.target.value))}
                className="w-full max-w-xs p-2 rounded-md bg-gyn-bg-primary-light dark:bg-gyn-bg-primary-dark border-2 border-gyn-border-primary-light dark:border-gyn-border-primary-dark"
                step="0.1"
              />
            </div>
          </SettingsSection>
        )}
      
        {activeTab === 'integrations' && (
          <SettingsSection
            title="Ticketing Integrations"
            description="Connect with third-party ticketing providers."
          >
            <div className="py-2">
              <label className="font-semibold text-gyn-text-secondary-light dark:text-gyn-text-secondary-dark" htmlFor="ticketmaster-api">Ticketmaster API Key</label>
              <p className="text-sm mb-2">Enter your Ticketmaster API key to feature affiliate ticket links.</p>
              <input
                id="ticketmaster-api"
                type="text"
                placeholder="Enter API Key..."
                value={settings.ticketmasterApi}
                onChange={(e) => handleChange('ticketmasterApi', e.target.value)}
                className="w-full max-w-sm p-2 rounded-md bg-gyn-bg-primary-light dark:bg-gyn-bg-primary-dark border-2 border-gyn-border-primary-light dark:border-gyn-border-primary-dark"
              />
            </div>
          </SettingsSection>
        )}
      </div>

      <div className="flex justify-end mt-8">
        <button onClick={handleSave} className="px-6 py-2 bg-gyn-sidebar-light dark:bg-gyn-sidebar-dark text-gyn-sidebar-text-light dark:text-gyn-sidebar-text-dark font-bold rounded-md hover:bg-gyn-sidebar-interactive-light dark:hover:bg-gyn-sidebar-interactive-dark transition-colors">
            Save Changes
        </button>
      </div>
    </div>
  );
};

export default EventsSettingsView;
